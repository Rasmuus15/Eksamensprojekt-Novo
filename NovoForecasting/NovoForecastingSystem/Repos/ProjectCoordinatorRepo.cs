...
namespace NovoForecastingSystem.Repos
{
    public class ProjectCoordinatorRepo : DatabaseConnector
    {
        ...
        public void CreatePC(ProjectCoordinator pcToBeCreated)
        {
            using (SqlConnection connection = new SqlConnection(connectionString))
            {
                                                       ...
                                                       "SELECT SCOPE_IDENTITY();", connection))
                {
                    ...
                    cmd.Parameters.Add("@Initials", SqlDbType.NVarChar).Value = pcToBeCreated;
        ...
        public ProjectCoordinator? GetPCById( int coordinatorId)
        {
            ...
            {
                connection.Open ();
                SqlCommand cmd = new SqlCommand("SELECT CoordinatorId, Initials FROM ProjectCoordinator WHERE CoordinatorId = @CoordinatorId", connection);
